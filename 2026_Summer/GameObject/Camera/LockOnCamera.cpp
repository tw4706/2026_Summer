...
void LockOnCamera::Update(int stageModelHandle)
{
	...
	Vector3 targetPos = playerCameraPos + (directionVec * 350.0f) + Vector3{0.0f,50.0f,0.0f};

	//線形補間を行い、滑らかな追従を行う
	pos_ = Vector3::Lerp(pos_, targetPos, 0.1f);

	//カメラとステージの消灯判定を行う
	pos_ = CheckCollCameraToStage(stageModelHandle, playerCameraPos, pos_);

	DrawFormatString(0, 100, GetColor(255, 0, 0), L"Cam Pos: %.2f, %.2f, %.2f", pos_.x_, pos_.y_, pos_.z_);
