...
void BossCamera::Init()
{
    ...
    Vector3 sideOffset = Vector3{ -dir.z_, 0.0f, dir.x_ } *(dist * 0.5f + 500.0f);
...
void BossCamera::Update(int stageModelHandle)
{
    ...
    //プレイヤーとボスを結ぶ向きから、少し引いた位置にカメラを置く
    Vector3 diff = bossPos - playerPos;
    diff.y_ = 0.0f;
    float dist = diff.Length();
    Vector3 dir = (dist > 0.0001f) ? diff.Normalize() : Vector3{ 0.0f, 0.0f, 1.0f };
    ...
    //中間点から横に引いた位置+高さを設定
    Vector3 sideOffset = Vector3{ -dir.z_, 0.0f, dir.x_ } *(dist * 0.5f + 500.0f);
    Vector3 targetPos = targetCameraTarget + sideOffset + Vector3{ 0.0f, 300.0f, 0.0f };

    //現在値から目標値へ線形補間(Vector3::Lerpを使用)
...
void BossCamera::SetTarget(std::weak_ptr<class Player> pPlayer, std::weak_ptr<class EnemyBase> pBoss, CameraManager* pCameraManager)
{
    pPlayer_ = pPlayer;
    pBoss_ = pBoss;
